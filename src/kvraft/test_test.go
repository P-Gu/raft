...
// and in order
func checkClntAppends(t *testing.T, clnt int, v string, count int, key string) {
	lastoff := -1
	for j := 0; j < count; j++ {
		wanted := "x " + strconv.Itoa(clnt) + " " + strconv.Itoa(j) + " y"
		off := strings.Index(v, wanted)
		if off < 0 {
			t.Fatalf("%v missing element %v in Append result %v of key %v", clnt, wanted, v, key)
		}
		off1 := strings.LastIndex(v, wanted)
		if off1 != off {
			t.Fatalf("duplicate element %v in Append result of key %v", wanted, key)
		}
		if off <= lastoff {
			t.Fatalf("wrong order for element %v in Append  of key %v", wanted, key)
		}
		lastoff = off
	}
}
...
func GenericTestSpeed(t *testing.T, part string, maxraftstate int) {
	...
	v := ck.Get("x")
	checkClntAppends(t, 0, v, numOps, "x")
